...
"""Tokenization, audio loading, message parsing, and batched collation for raon model inputs."""
...
from ..models.raon import RaonConfig
...
# ---------------------------------------------------------------------------
# Prompt constants and augmentation helpers
# ---------------------------------------------------------------------------
...
def detect_task_type(channel: str) -> str:
    """Detect task type from the channel field."""
    if "tts" in channel:
        return "tts"
    elif "stt" in channel:
        return "stt"
    elif "speech-chat" in channel:
        return "speech-chat"
    elif "textqa" in channel:
        return "textqa"
    return "unknown"


def augment_stt_messages(messages: list) -> None:
    """Merge STT transcription instruction into the last user audio turn (in-place)."""
    prompt = get_default_stt_prompt()
    for j in range(len(messages) - 1, -1, -1):
        if messages[j].get("role") == "user":
            ...
            if isinstance(content, list):
                ...
                messages[j] = {**messages[j], "content": content}
            elif isinstance(content, str):
                messages[j] = {**messages[j], "content": f"{content}{prompt}"}
            break


def augment_tts_messages(
    messages: list,
    use_speaker_embedding: bool = False,
) -> None:
    """Prepend TTS instruction (+ optional speaker token) to first user turn (in-place)."""
    if not messages:
        return
    speaker_token = str(SPEAKER_EMBEDDING_PLACEHOLDER) if use_speaker_embedding else ""
    prompt = get_default_tts_prompt()
    first = messages[0]
    content = first.get("content", "")
    if isinstance(content, str):
        messages[0] = {**first, "content": f"{speaker_token}{prompt}:\n{content}"}
    elif isinstance(content, list):
        messages[0] = {**first, "content": [{"type": "text", "text": f"{speaker_token}{prompt}:\n"}] + content}


def resolve_audio_paths(audios: list[str]) -> list[str]:
    """Return the audio paths from the JSONL ``audios`` field.

    Args:
        audios: Audio file paths from the JSONL ``audios`` field.

    Returns:
        List of audio paths (shallow copy).
    """
    return list(audios) if audios else []


def convert_to_multimodal(
    message: dict[str, Any],
    audio_paths_iter: Any,
) -> dict[str, Any]:
    """Replace ``<audio>`` tags in a message with audio content items.

    Matches duplex-model ``PretrainingSequence.convert_to_multimodal``:
    splits message content on ``<audio>`` tags and interleaves text/audio
    content items, consuming one path from *audio_paths_iter* per tag.

    Args:
        message: A single message dict with ``role`` and ``content`` keys.
        audio_paths_iter: Iterator yielding audio file paths.

    Returns:
        The original message if no ``<audio>`` tags are present, otherwise
        a new message with multimodal content items.
    """
    content = message.get("content", "")
    if not isinstance(content, str) or PRETRAINING_AUDIO_TAG not in content:
        return message

    content_items: list[dict[str, str]] = []
    parts = content.split(PRETRAINING_AUDIO_TAG)
    for i, part in enumerate(parts):
        if part:
            content_items.append({"type": "text", "text": part})
        if i < len(parts) - 1:
            content_items.append({"type": "audio", "audio": next(audio_paths_iter)})

    return {**message, "content": content_items}


# ---------------------------------------------------------------------------
# Processor types and RaonProcessor
# ---------------------------------------------------------------------------
